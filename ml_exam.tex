...
The paired bootstrap test does not produce a true p-value because of a fundamental mismatch between the distribution it estimates and the distribution required for valid hypothesis testing.

\subsection*{The Core Problem}

In proper statistical hypothesis testing, the p-value is defined as the probability of observing a test statistic at least as extreme as the one computed from the data, \textbf{assuming the null hypothesis is true}. This requires knowing (or estimating) the distribution of the test statistic \textbf{under the null hypothesis} $H_0$.

However, the paired bootstrap test estimates the distribution of the performance difference by resampling from the \textbf{original (true) data distribution}, not from a distribution where the null hypothesis holds. Specifically:

\begin{itemize}
    \item \textbf{Bootstrap distribution:} The bootstrap resampling procedure generates samples from the empirical distribution of the observed data. This distribution reflects the \emph{actual} performance difference between the two models.
    \item \textbf{Required distribution for p-value:} A valid p-value requires the distribution of the test statistic under $H_0$ (i.e., assuming no difference between the models, or that any observed difference is due to random chance).
\end{itemize}

\subsection*{Why This Matters}

The null hypothesis typically states that the two models perform equally well. Under $H_0$, we would expect the distribution of performance differences to be centered around zero. However, the bootstrap procedure samples from the observed data, which already contains the true (potentially non-zero) performance difference between the models.

As a result, the ``p-value'' computed from the paired bootstrap test is not calibrated correctly:
\begin{itemize}
    \item It estimates the probability of observing a certain difference given the \emph{true underlying distribution}, not given that $H_0$ is true.
    \item The bootstrap distribution is centered around the \emph{observed} difference, not around zero (as required under $H_0$).
\end{itemize}

\subsection*{Contrast with Permutation Test}

The random permutation test, by contrast, \emph{does} produce a valid p-value. By randomly permuting the model predictions (swapping which prediction came from which model), it simulates the distribution of the test statistic under the null hypothesis that the two models are exchangeable (i.e., equally good). This gives a proper reference distribution for computing the p-value.

\subsection*{Practical Usefulness}

Despite not producing a true p-value, the paired bootstrap test can still be useful for:
\begin{itemize}
    \item Estimating confidence intervals for the performance difference.
    \item Getting a rough sense of the variability in model comparison.
    \item Quick exploratory analysis when exact p-values are not critical.
\end{itemize}

However, for rigorous statistical claims about model superiority, the permutation test or other methods that properly estimate the null distribution should be preferred.
